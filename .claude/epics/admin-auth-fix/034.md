...
## Criterio Done
- [ ] Tipo `VerifyError` incluye `status: 503`
- [ ] Error de DB en el query de roles retorna `{ error: 'Service Unavailable', status: 503 }`
- [ ] `console.error` en dev con mensaje del error
- [ ] Type assertion removido (tipos inferidos correctamente)
- [ ] TypeScript compila sin errores: `npx tsc --noEmit` no muestra errores en archivos admin
